...
### Fixed
- OneUI freeform caption maximize (looks like fullscreen) then close from the task stack: reopen stayed maximized (no caption / split handle) because `ensureFreeform` skipped existing FREEFORM; phone↔VD round-trip restored inset freeform but also suppressed bounce for 2s so the next split click failed. Launch / create / move-to-VD now demote nearly-fullscreen freeform bounds, clear bounce suppress when moving onto the VD, and force-clean empty stage shells.
