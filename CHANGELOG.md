...
### Fixed
- OneUI freeform caption swipe-down on the AA VD: minimize (`isMinimized=true`) or drag mostly off-screen made the app vanish, and stack tap via plain `moveTaskToFront`/`setFocusedTask` could not restore it. Now auto-restores (and stack tap restores) via FREEFORM relaunch + inset `resizeTask`.
