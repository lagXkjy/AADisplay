...
### Fixed
- **Left rail strip still untouchable after reclaim (r44–r48):** ADB on SM-W7023 + AA 17.4 showed Coolwalk still routes HU `x∈[0,80]` to `GhFacetBar` (separate VD). Prior `1px` shrink + `FLAG_NOT_TOUCHABLE` made the strip look filled but dropped every event there (direct inject into `AaDisplayActivity` left edge worked). **r44:** keep named FacetBar at rail width, paint window `alpha=0`, relay via `ICoreManager.touchHost`. **r45:** intercept Coolwalk `injectTouchEvent` → `touchHost`. **r46:** keep FacetBar `VISIBLE` (had been `GONE`/`NO_SURFACE`). **r47:** Samsung treats `alpha=0` as InputDispatcher `NOT_VISIBLE`, so Coolwalk `imt.H` logs "does not correspond to a window" and never calls inject; steal LHD `x<rail` at that HU dispatcher → `touchHost` (rail width from live FacetBar/`content_bounds`, else ~10% of LayoutInfo HU width; RHD out of scope), and stop forcing window alpha=0. Always pin LayoutInfo to the LHD vertical-rail family. **r48:** those HU/inject hooks were only installed in `:projection`, but `CarActivityManagerService` (`imt.H/G/I`) runs in `:car` — r47 never saw a single left-rail event. Hook `:car` as well, cover the MotionEvent overloads, probe private host display ids (getDisplays hides them), and fall back to injecting PRIMARY pane when host inject no-ops. **r49:** r48 did steal HU `x<80` (`HU rail → touchHost`) but `BridgeService` only hands the CoreManager binder to the AADisplay app uid; `:car` (gearhead) was rejected (`Someone else trying to get my binder?`) so inject never ran. Allow gearhead uid, always inject PRIMARY pane (host VD is `touch NONE`), and synthesize MotionEvents with `uptimeMillis`.
