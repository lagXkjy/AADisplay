...
| 仪表横条歌词（AA Title） | `xposed/cluster/ClusterLyricMirror.kt`、`ShadowNowPlayingSession.kt`、`LyricLineExtractor.kt`；`service/ClusterLyricMediaService.kt`（`:cluster`）；gearhead `AaMediaAllowlistHook` / `AaClusterLyricEgressHook` |
