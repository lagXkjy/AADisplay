...
- 当前模块版本：`0.24#17.4-r13`（`versionCode` 3073）
- 变更记录见 [CHANGELOG.md](CHANGELOG.md)
- 本版用户说明见 [RELEASE_NOTES_0.24-17.4-r10.md](RELEASE_NOTES_0.24-17.4-r10.md)（后续修复见 CHANGELOG Unreleased）
...
4. **无桌面图标**：激活状态页从 LSPosed → AADisplay 打开；手机侧悬浮框已移除。

### 亮点

1. **精简导航栏**，更充分利用车机屏幕。
2. **自定义分屏**，不再依赖系统分屏壳；支持双 VD 全屏（一显一隐）与贴边 peel 把手。
3. **每窗最多 3 个应用保活**：选新应用不再直接杀掉旧的；Recent 点选即可快速切顶，状态保持。
4. **Task 应用栈**为 **3 列**：左 → 虚拟屏左，中 → 虚拟屏右，右 → 手机；卡片有 Close，左/中列底部可「添加应用」。
5. **交互入口集中在分屏条上的三个小点**：
   - **长按**：进入 Task 应用栈（全屏 peel 为松手后再打开）
   - **点击**：交换左右分屏（全屏时点 peel 切换可见侧）
   - **拖动**：调整分屏比例（拖过边缘可进全屏；全屏向内拖可退出）
6. **分屏窗口记忆**：自动记忆左右栈序、比例与全屏状态，下次连接车机自动恢复。
