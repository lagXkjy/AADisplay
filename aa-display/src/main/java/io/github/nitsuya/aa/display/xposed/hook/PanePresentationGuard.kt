 ...
 *
 * Safety: no AA session → no-op; callback failures never throw into WMS.
 */
object PanePresentationGuard {
    ...
    @Volatile private var sessionGetUidMethod: Method? = null
    @Volatile private var sessionGetUidLookupDone = false
    ...
    private fun installHooks() {
        runCatching { hookAddWindow() }
            .onFailure { log(TAG, "addWindow install failed", it) }
        runCatching { hookAttachWindowContext() }
            .onFailure { log(TAG, "attachContext install failed", it) }
    ...
    private fun hookAddWindow() {
        ...
        addWindowHook = method.hookBefore { param ->
            try {
                if (!CoreManagerService.hasAaVirtualDisplays()) return@hookBefore
    ...
    private fun sessionUid(session: Any): Int? {
        val method = sessionGetUidMethod ?: if (sessionGetUidLookupDone) {
            null
        } else {
            sessionGetUidLookupDone = true
            session.javaClass.methods.firstOrNull { m ->
                m.name == "getUid" && m.parameterTypes.isEmpty()
            }?.also {
                it.isAccessible = true
                sessionGetUidMethod = it
            }
        } ?: return null
        return try {
            method.invoke(session) as? Int
        } catch (_: Throwable) {
            null
        }
    }
